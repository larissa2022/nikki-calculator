...
const decide = async (item, decision) => {
  ...
  let reason = null
  if (decision === 'reject') {
    reason = item.reject_reason || window.prompt(`请填写驳回《${item.name}》的原因。另一位管理员需要填写相同原因。`)?.trim()
    if (!reason) return
  } else if (!window.confirm(`确认通过套装《${item.name}》吗？`)) {
    return
  }
  ...
  try {
    ...
    notice.value = result?.status === 'awaiting_cosign'
      ? `你的决定已记录（${result.signature_count} / ${result.required_signatures}）。还需要另一位管理员做出相同决定。`
      : `《${item.name}》已${result?.decision === 'rejected' ? '驳回' : '通过'}。`
    await load()
  } catch (error) {
...
<template>
  <section class="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm md:p-6">
    <div class="mb-5 flex flex-wrap items-center justify-between gap-3">
      <div>
        <h3 class="text-xl font-black text-slate-800">📦 套装审核</h3>
        <p class="mt-1 text-xs font-bold text-slate-500">
          {{ isSuperAdmin ? '站长可以直接通过或驳回。' : '需要两位管理员做出相同决定。第一位确认后，请换另一位管理员继续。' }}
        </p>
      </div>
        ...
        <div class="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
          <div>
            ...
            <div class="mt-2 flex flex-wrap gap-2 text-xs font-black">
              ...
              <span v-if="item.my_decision" class="rounded-full bg-purple-50 px-2 py-1 text-purple-700">我已选择{{ item.my_decision === 'approve' ? '通过' : '驳回' }}</span>
            </div>
            <p v-if="item.reject_reason" class="mt-2 text-xs font-bold text-rose-600">现有驳回提案：{{ item.reject_reason }}</p>
          </div>
          <div class="flex shrink-0 gap-2">
            <button class="rounded-lg border border-rose-200 px-4 py-2 text-sm font-black text-rose-600 disabled:opacity-50" :disabled="Boolean(activeName) || item.my_decision === 'approve'" @click="decide(item, 'reject')">确认驳回</button>
            <button class="rounded-lg bg-blue-500 px-4 py-2 text-sm font-black text-white disabled:opacity-50" :disabled="Boolean(activeName) || item.my_decision === 'reject'" @click="decide(item, 'approve')">确认通过</button>
          </div>
        </div>
      </article>
    </div>
