# 历史文档归档

本目录保存已经结束、仍具有审计价值，但不再参与当前任务执行的历史材料。

## 使用边界

- 本目录不是 AI / Codex 任务入口。
- 归档内容可能反映旧分支、旧任务、旧文件结构或当时的待确认事项。
- 当前任务以 `docs/ai/CURRENT_TASK.md` 为准。
- 强门禁以 `docs/ai/RULES.md` 为准。
- 当前操作流程以 `docs/ai/WORKFLOWS.md` 为准。
- 产品与技术 Final 决策以 `docs/ai/DECISIONS.md` 为准。
- 归档材料不得自动恢复为当前规则、当前计划或执行依据。

## 当前归档

| 文件 | 原位置 | 定位 |
| --- | --- | --- |
| [Codex 历史执行报告](CODEX_REPORT.md) | `docs/ai/CODEX_REPORT.md` | 早期项目状态、文档收口和执行结果的历史汇总 |
| [Token 使用审计](TOKEN_USAGE_AUDIT.md) | `docs/ai/TOKEN_USAGE_AUDIT.md` | 2026-07-12 一次性上下文与 Token 成本审计 |

需要追溯完整历史时，以 Git commit、PR 和原文件 blob 为证据，不从归档内容推断当前状态。
