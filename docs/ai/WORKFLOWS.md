...
- 当前任务看板：[`CURRENT_TASK.md`](CURRENT_TASK.md)。
- 产品与技术 Final 决策：[`DECISIONS.md`](DECISIONS.md)。
- 若本文与 `RULES.md` 冲突，以 `RULES.md` 为准。

读取路由只在 `AGENTS.md` 中定义，不在本文维护第二套默认必读清单。

## 1. CURRENT_TASK.md 模板与填写规则

`CURRENT_TASK.md` 是由当前任务窗口维护的一屏任务看板，不是长期路线图、Git 操作日志或新任务启动教程。
...
- 业务目标由当前任务窗口根据本轮需求文档提炼。
- 具体产品规则以 `DECISIONS.md` 中的 Final 决策为准。
...
- 不记录命令流水、短时 PR 状态、长期历史、相关文档列表或详细技术说明。
- 超出一屏的内容移入需求、规划、缺陷或数据库记录文档，看板只保留摘要。

### 1.2 无活动任务状态

没有活动任务时不预设业务目标、技术目标或下一条业务主线，只保留：当前无活动任务、通用边界和 Rollback。具体内容由下一个任务窗口在读取本轮需求并取得确认后填写。

看板只在业务状态、技术阶段或下一步任务发生实质变化时更新，不因 PR 从 open 变为 merged 单独创建状态 PR。
...
- `CURRENT_TASK.md` 只记录稳定的当前任务事实，不记录短时 PR 状态或预设的新业务主线。
