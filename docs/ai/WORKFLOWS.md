...
- 最近完成只保留与当前任务直接相关的近期结果。
- 不记录命令流水、短时 PR 状态、长期历史、相关文档列表或详细技术说明；超出一屏的内容移入对应需求、规划、缺陷或数据库记录文档。
- 没有活动任务时不预设业务目标、技术目标或下一条业务主线，只保留“当前无活动任务”、通用边界和 Rollback。
...
成功时只返回结论、数量和异常；失败时只提供定位所需的关键错误。回传中的“下一步任务”必须与看板一致。无数据库、Vercel 或 production 变更时明确说明。
