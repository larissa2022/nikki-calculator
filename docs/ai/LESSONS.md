...
- 处理方式：新增统一验收清单模板，保留验收范围、已准备数据、最短验收流程、总通过标准、异常反馈格式、数据保留与清理和 Rollback；移除账号密码总表与放置规则模块。每个账号使用独立的“一次登录”测试模块，模块内明确填写账号和 8 至 16 位英文字母数字简单密码；校验器逐模块检查并拒绝统一密码、模块外凭据和复杂随机密码。
