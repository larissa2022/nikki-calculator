...
- 日期：2026-07-20
- 来源任务：DB-2 开发、PR #82 / #83 / #84 合并与合并后收口
- 现象：
...
- 原因：
  ...
  - 执行侧：PR #82 的预合并审计没有识别看板会在合并后立即过期，这是造成 PR #84 的直接遗漏；等待授权期间重复回读无变化状态、单个长对话承载过多独立阶段，也放大了重复沟通和断流影响。
  - 仓库与工具侧：并行 PR 修改重叠文档形成基线变化；远端连接失败的结果不确定，必须先回读远端事实，不能直接把报错等同于操作失败。
- 处理方式：
  - 五项执行单中的停止点改为可验证边界；“继续 / 下一步”存在歧义时，以看板第一条下一步为准，并在启动回执中声明选择。
  - 新增看板合并后稳定性检查：修改看板的 PR 在请求 merge 前不得保留只对合并前成立的短时状态。
...
- Rule Candidate：暂无；先在 `WORKFLOWS.md` 固化为流程并观察复用效果。
- 状态：可复用
