...
begin
  ...
  if v_pending_status_count = 0 and v_approved_status_count = v_requested_pending_count then
    ...
    from pg_catalog.unnest(
      ...
      v_effective_pending_ids,
      v_effective_created_ats
    ) with ordinality as expected(user_id, source_pending_id, source_created_at, contribution_rank)
    join public.clothing_contributions as contribution
      ...
      and contribution.user_id = expected.user_id
      and contribution.source_pending_id = expected.source_pending_id
      and contribution.source_created_at = expected.source_created_at
      and contribution.contribution_rank = expected.contribution_rank::smallint
...
comment on function public.complete_existing_clothes_from_pending(
  text, text, text, text, integer, jsonb, uuid, text, text, bigint[]
) is 'DB-3：正式库空字段补全，同事务记录前 5 位有效贡献者、每人 5 分、衣柜写回和 pending 通过；重复请求幂等。';
